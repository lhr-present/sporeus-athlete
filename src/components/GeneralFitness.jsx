...
// Manages tab state: Dashboard | Log | Program | Progress
...
const INNER_TABS = [
  { id: 'dashboard', en: 'DASHBOARD', tr: 'PANO' },
  { id: 'log',       en: 'LOG',       tr: 'KAYIT' },
  { id: 'program',   en: 'PROGRAM',   tr: 'PROGRAM' },
  { id: 'progress',  en: 'PROGRESS',  tr: 'İLERLEME' },
]

// ── Seed program templates (client-side fallback for offline / no-Supabase) ──
const STATIC_TEMPLATES = [
  ...
  { id:'ppl_3day_beginner',     name_en:'Push/Pull/Legs 3-Day',            name_tr:'İtiş/Çekiş/Bacak 3 Gün',        split:'ppl',          days_per_week:3, experience_level:'beginner',     equipment:'gym',  weeks:4, description_en:'Push, Pull, Legs on 3 days — one rotation per week. Simple and effective for muscle building.',     description_tr:'Haftada 3 gün İtiş, Çekiş, Bacak. Kas gelişimi için sade ve etkili.' },
  { id:'ppl_6day_intermediate', name_en:'Push/Pull/Legs 6-Day',            name_tr:'İtiş/Çekiş/Bacak 6 Gün',        split:'ppl',          days_per_week:6, experience_level:'intermediate', equipment:'gym',  weeks:4, description_en:'Two full PPL rotations per week. High frequency hypertrophy for intermediate athletes.',             description_tr:'Haftada iki tam İtiş/Çekiş/Bacak turu. Orta seviye için yüksek frekans hipertrofi.' },
  { id:'home_db_3day',          name_en:'Home Dumbbell 3-Day',             name_tr:'Evde Dumbbell 3 Gün',            split:'full_body',    days_per_week:3, experience_level:'beginner',     equipment:'home', weeks:4, description_en:'Full body training with dumbbells and resistance bands only. No gym required.',                    description_tr:'Sadece dumbbell ve direnç bandıyla tüm vücut antrenmanı. Spor salonu gerekmez.' },
  { id:'home_db_4day',          name_en:'Home Dumbbell 4-Day',             name_tr:'Evde Dumbbell 4 Gün',            split:'upper_lower',  days_per_week:4, experience_level:'intermediate', equipment:'home', weeks:4, description_en:'Upper/lower split using dumbbells and bands. Step up from the 3-day home program.',                description_tr:'Dumbbell ve bantla üst/alt ayrımı. 3 günlük ev programından bir adım yukarı.' },
  { id:'recomp_4day',           name_en:'Recomp 4-Day',                    name_tr:'Rekomp 4 Gün',                   split:'upper_lower',  days_per_week:4, experience_level:'beginner',     equipment:'gym',  weeks:4, description_en:'Accessible volume for fat-loss with muscle retention. Lower session volume, higher frequency.',    description_tr:'Yağ yakarken kas koruma için uygun hacim. Düşük seans hacmi, yüksek frekans.' },
]
...
const SEED_EXERCISES = [
  ...
  { id:'db_ohp',           name_en:'DB Shoulder Press',       name_tr:'Dumbbell Omuz Press',      pattern:'push_v', primary_muscle:'delts',      secondary_muscles:['triceps'],                     equipment:'db', is_compound:false, cues_en:'DBs at ear level, press overhead, don\'t flare elbows excessively.',                   cues_tr:'DB\'ler kulak hizasında, baş üstüne it, dirsekleri aşırı açma.' },
  ...
  { id:'db_lateral_raise', name_en:'DB Lateral Raise',        name_tr:'Dumbbell Yan Kaldırma',    pattern:'iso',    primary_muscle:'delts',      secondary_muscles:[],                              equipment:'db', is_compound:false, cues_en:'Slight forward lean, raise DBs to shoulder height, control descent.',                  cues_tr:'Hafif öne eğim, DB\'leri omuz hizasına kaldır, kontrollü in.' },
  ...
  { id:'db_calf_raise',    name_en:'DB Calf Raise',           name_tr:'Dumbbell Baldır Kaldırma', pattern:'iso',    primary_muscle:'calves',     secondary_muscles:[],                              equipment:'db', is_compound:false, cues_en:'Hold DBs, rise on toes, squeeze calves at top.',                                       cues_tr:'DB\'leri tut, parmak uçlarına yüksEl, üstte baldırları sık.' },
  // Bodyweight
  ...
  { id:'bw_incline_push',  name_en:'Incline Push-Up',         name_tr:'Eğimli Şınav',             pattern:'push_h', primary_muscle:'chest',      secondary_muscles:['triceps'],                     equipment:'bw', is_compound:false, cues_en:'Hands on elevated surface, easier than flat. Good beginner entry.',                    cues_tr:'Eller yükseltilmiş yüzeyde, düzden daha kolay. Başlangıç için iyi.' },
  ...
  { id:'bw_pullup',        name_en:'Pull-Up',                 name_tr:'Barfiks',                  pattern:'pull_v', primary_muscle:'back',       secondary_muscles:['biceps'],                      equipment:'bw', is_compound:true,  cues_en:'Hang at dead hang, pull chest to bar, full ROM.',                                      cues_tr:'Ölü asılış, göğsü bara çek, tam hareket açıklığı.' },
  { id:'bw_chin_up',       name_en:'Chin-Up',                 name_tr:'Ters Barfiks',             pattern:'pull_v', primary_muscle:'biceps',     secondary_muscles:['back'],                        equipment:'bw', is_compound:true,  cues_en:'Underhand grip, pull chin over bar, squeeze biceps at top.',                           cues_tr:'Ters tutuş, çeneyi barın üstüne çek, üstte bisepsi sık.' },
  { id:'bw_bodyweight_sq', name_en:'Bodyweight Squat',        name_tr:'Vücut Ağırlığı Squat',     pattern:'squat',  primary_muscle:'quads',      secondary_muscles:['glutes'],                      equipment:'bw', is_compound:true,  cues_en:'Feet shoulder-width, toes out slightly, descend to depth, knees track toes.',          cues_tr:'Ayaklar omuz genişliğinde, parmaklar hafif dışa, derine in.' },
  { id:'bw_glute_bridge',  name_en:'Glute Bridge',            name_tr:'Kalça Köprüsü',            pattern:'hinge',  primary_muscle:'glutes',     secondary_muscles:['hamstrings'],                  equipment:'bw', is_compound:false, cues_en:'Lie on back, feet flat, drive hips up, squeeze glutes at top.',                        cues_tr:'Sırt üstü yat, ayaklar düz, kalçaları kaldır, üstte sık.' },
  ...
  { id:'bw_nordic_curl',   name_en:'Nordic Curl',             name_tr:'Nordik Curl',              pattern:'hinge',  primary_muscle:'hamstrings', secondary_muscles:[],                              equipment:'bw', is_compound:true,  cues_en:'Kneel, partner holds ankles, lower slowly, catch at bottom.',                          cues_tr:'Diz çök, partner ayak bileklerini tutsun, yavaşça in, altta tut.' },
  // Machines
  { id:'machine_leg_press',  name_en:'Leg Press',             name_tr:'Bacak Presi',              pattern:'squat',  primary_muscle:'quads',      secondary_muscles:['glutes','hamstrings'],          equipment:'machine', is_compound:true,  cues_en:'Feet hip-width, knees track toes, don\'t lock out fully.',                            cues_tr:'Ayaklar kalça genişliğinde, dizler parmak ucunu takip etsin, tam kilitleme.' },
  { id:'machine_leg_curl',   name_en:'Lying Leg Curl',        name_tr:'Yatay Bacak Curl',         pattern:'hinge',  primary_muscle:'hamstrings', secondary_muscles:[],                              equipment:'machine', is_compound:false, cues_en:'Hips down, curl pad toward glutes, controlled extension.',                            cues_tr:'Kalçalar aşağı, pedi kalçalara doğru kıv, kontrollü uzatma.' },
  { id:'machine_leg_ext',    name_en:'Leg Extension',         name_tr:'Bacak Uzatma',             pattern:'iso',    primary_muscle:'quads',      secondary_muscles:[],                              equipment:'machine', is_compound:false, cues_en:'Full extension at top, slow eccentric to protect patellar tendon.',                    cues_tr:'Tepede tam uzatma, patellar tendonu korumak için yavaş eksantrik.' },
  { id:'machine_cable_row',  name_en:'Seated Cable Row',      name_tr:'Oturarak Kablo Kürek',     pattern:'pull_h', primary_muscle:'back',       secondary_muscles:['biceps'],                      equipment:'cable', is_compound:false, cues_en:'Pull to navel, elbows back, squeeze lats, controlled return.',                         cues_tr:'Göbeğe çek, dirsekler geri, lat\'ı sık, kontrollü dön.' },
  { id:'machine_lat_pulldown',name_en:'Lat Pulldown',         name_tr:'Lat Pulldown',             pattern:'pull_v', primary_muscle:'back',       secondary_muscles:['biceps'],                      equipment:'cable', is_compound:false, cues_en:'Wide grip, pull to upper chest, slight lean back, full stretch at top.',               cues_tr:'Geniş tutuş, üst göğse çek, hafif geriye eğil, üstte tam gerim.' },
  { id:'machine_cable_fly',  name_en:'Cable Crossover Fly',   name_tr:'Kablo Crossover',          pattern:'push_h', primary_muscle:'chest',      secondary_muscles:[],                              equipment:'cable', is_compound:false, cues_en:'High or mid pulley, squeeze chest at bottom of arc.',                                  cues_tr:'Yüksek veya orta makara, yayın altında göğsü sık.' },
  { id:'machine_facepull',   name_en:'Face Pull',             name_tr:'Yüz Çekme',                pattern:'pull_h', primary_muscle:'delts',      secondary_muscles:['back'],                        equipment:'cable', is_compound:false, cues_en:'Rope at face level, pull to forehead, elbows flare, external rotation.',               cues_tr:'Halat yüz hizasında, alna çek, dirsekler açılsın, dış rotasyon.' },
  { id:'machine_pushdown',   name_en:'Tricep Pushdown',       name_tr:'Triseps Baskısı',          pattern:'push_v', primary_muscle:'triceps',    secondary_muscles:[],                              equipment:'cable', is_compound:false, cues_en:'Rope or bar, elbows fixed, extend arms fully, control return.',                        cues_tr:'Halat veya bar, dirsekler sabit, kolları tam uzat, kontrollü dön.' },
  { id:'machine_curl_cable', name_en:'Cable Curl',            name_tr:'Kablo Biseps Curl',        pattern:'pull_v', primary_muscle:'biceps',     secondary_muscles:[],                              equipment:'cable', is_compound:false, cues_en:'Straight or EZ bar, curl to chin level, no swinging.',                                cues_tr:'Düz veya EZ bar, çene seviyesine kadar kaldır, sallanma.' },
  // Band
  ...
  { id:'band_squat',         name_en:'Band Squat',            name_tr:'Bant Squat',               pattern:'squat',  primary_muscle:'quads',      secondary_muscles:['glutes'],                      equipment:'band', is_compound:true,  cues_en:'Band under feet, handles at shoulder level, squat to depth.',                         cues_tr:'Bant ayakların altında, kollar omuz hizasında, derinliğe in.' },
  { id:'band_row',           name_en:'Band Seated Row',       name_tr:'Bant Oturarak Kürek',      pattern:'pull_h', primary_muscle:'back',       secondary_muscles:['biceps'],                      equipment:'band', is_compound:false, cues_en:'Wrap band around feet, pull to navel, squeeze shoulder blades.',                       cues_tr:'Bandı ayakların etrafına sar, göbeğe çek, kürek kemiklerini sık.' },
]

export default function GeneralFitness({ lang = 'en' }) {
  const t = (en, tr) => lang === 'tr' ? tr : en

  const [innerTab, setInnerTab]       = useLocalStorage('sporeus-gf-tab', 'dashboard')
  const [onboarded, setOnboarded]     = useLocalStorage('sporeus-gf-onboarded', false)
  const [gfProfile, setGfProfile]     = useLocalStorage('sporeus-gf-profile', null)
  const [sessions, setSessions]       = useLocalStorage('sporeus-gf-sessions', [])
  const [activeTemplateId, setActiveTemplateId] = useLocalStorage('sporeus-gf-template', null)
  const [showLogger, setShowLogger]   = useState(false)

  const activeTemplate = STATIC_TEMPLATES.find(t => t.id === activeTemplateId) ?? null
  ...
  function handleSaveSession(session) {
    const entry = { ...session, id: crypto.randomUUID?.() ?? Date.now().toString(), session_date: new Date().toISOString().slice(0, 10) }
  ...
  if (!onboarded) {
    return (
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        <div style={{ ...S.mono, fontSize: 14, color: '#ff6600', letterSpacing: '0.12em', marginBottom: 4, padding: '24px 16px 0' }}>
          {t('GENERAL FITNESS', 'GENEL KONDİSYON')}
        </div>
        <div style={{ ...S.mono, fontSize: 10, color: '#888', padding: '0 16px 16px', letterSpacing: '0.06em' }}>
          {t('Set up your program in 4 steps.', '4 adımda programını oluştur.')}
        </div>
      ...
      <div role="tablist" style={{ display: 'flex', gap: 4, marginBottom: 16, overflowX: 'auto' }}>
        {INNER_TABS.map(tab => (
          <button
            ...
            style={{ ...S.mono, fontSize: 10, padding: '6px 12px', border: `1px solid ${innerTab === tab.id ? '#ff6600' : 'var(--border)'}`, background: innerTab === tab.id ? '#ff660022' : 'transparent', color: innerTab === tab.id ? '#ff6600' : 'var(--muted)', borderRadius: 3, cursor: 'pointer', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}
        ...
        <button
          style={{ ...S.mono, fontSize: 10, padding: '6px 12px', border: '1px solid var(--border)', background: 'transparent', color: '#888', borderRadius: 3, cursor: 'pointer', marginLeft: 'auto' }}
          onClick={() => { setOnboarded(false); setGfProfile(null) }}
          title={t('Reset onboarding', 'Kurulumu sıfırla')}
        >⚙</button>
      </div>

      <ErrorBoundary>
        <Suspense fallback={null}>
          {innerTab === 'dashboard' && !showLogger && (
            <GeneralDashboard
              sessions={sessions}
              exercises={SEED_EXERCISES}
              activeProgram={gfProfile}
              templateDays={[]}
              lang={lang}
          ...
          {innerTab === 'progress' && !showLogger && (
            <div style={{ ...S.mono, fontSize: 11, color: '#555', padding: '24px 0' }}>
              {sessions.length < 2
                ? t('Log at least 2 sessions to see progression charts.', 'İlerleme grafiği için en az 2 seans kaydet.')
                : t('Progression data available. Switch to Dashboard.', 'İlerleme verisi mevcut. Panoya geç.')
              }
            </div>
          )}
        </Suspense>
