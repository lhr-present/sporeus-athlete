...
export default function GeneralInsights({ sessions = [], exercises = [], lang = 'en' }) {
  ...
  const weekSets = useMemo(() =>
    sessions
      ...
      .flatMap(s => (s.strength_sets ?? []).map(set => ({ ...set, rir: set.rir ?? 3, reps: set.reps ?? 0 }))),
    [sessions, weekStart]
  )

  const progressExercises = useMemo(() => {
    const ids = [...new Set(sessions.flatMap(s => (s.strength_sets ?? []).map(x => x.exercise_id)))]
    return ids.filter(id => sessions.filter(s => s.strength_sets?.some(x => x.exercise_id === id)).length >= 2).slice(0, 4)
  }, [sessions])
