...
export default function EliteMetricsStrip({ profile, log, testResults, isTR, onGoToProfile }) {
  ...
  if (metrics?.power?.wPerKg != null)
    items.push({ label: 'W/kg', value: metrics.power.wPerKg.toFixed(2) })
  if (metrics?.running?.vdot != null)
    items.push({ label: 'VDOT', value: metrics.running.vdot.toFixed(1) })
  if (metrics?.hr?.maxHR != null)
    items.push({ label: isTR ? 'MaksKA' : 'MaxHR', value: String(metrics.hr.maxHR) })
  if (metrics?.hr?.lthr != null)
    items.push({ label: 'LTHR', value: `${metrics.hr.lthr}` })
  // Auto-VDOT (when vo2max not set but derived from log)
  if (!metrics?.running?.vdot && metrics?.autoVdot?.vdot != null)
    items.push({ label: 'VDOT*', value: metrics.autoVdot.vdot.toFixed(1), auto: true })
  ...
  return (
    ...
    >
      {items.map((item) => (
        <span key={item.label} style={{ display: 'flex', alignItems: 'baseline', gap: '3px', marginRight: '16px' }}>
