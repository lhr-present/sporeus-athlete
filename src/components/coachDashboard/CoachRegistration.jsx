  ...
  return (
    <div className="sp-fade">
      <div style={{ ...S.card, borderLeft:'3px solid #0064ff' }}>
        <div style={{ ...S.cardTitle, color:'#0064ff', borderColor:'#0064ff44' }}>◈ {lang === 'tr' ? 'KOÇ PROFİLİNİZİ OLUŞTURUN' : 'SET UP YOUR COACH PROFILE'}</div>
        ...
        <div style={S.row}>
          <div style={{ flex:'1 1 200px' }}>
            <label style={S.label}>{lang === 'tr' ? 'ADINIZ *' : 'YOUR NAME *'}</label>
            <input style={S.input} placeholder="Hüseyin Akbulut" value={name} onChange={e => setName(e.target.value)}/>
          </div>
          <div style={{ flex:'1 1 200px' }}>
            <label style={S.label}>{lang === 'tr' ? 'E-POSTA (isteğe bağlı, kodu benzersiz yapar)' : 'EMAIL (optional, makes code unique)'}</label>
        ...
        <button style={{ ...S.btn, marginTop:'16px' }} onClick={handleGenerate} disabled={!name.trim() || generating}>
          {generating ? (lang === 'tr' ? 'Oluşturuluyor...' : 'Generating...') : (lang === 'tr' ? '◈ Davet Kodumu Oluştur' : '◈ Generate My Invite Code')}
        </button>
        {generatedId && (
          <div style={{ marginTop:'20px', padding:'14px 16px', background:'#0064ff11', border:'1px solid #0064ff44', borderRadius:'6px' }}>
            <div style={{ ...S.mono, fontSize:'10px', color:'#888', marginBottom:'6px', letterSpacing:'0.1em' }}>{lang === 'tr' ? 'KOÇ KİMLİĞİNİZ' : 'YOUR COACH ID'}</div>
            ...
            <button style={{ ...S.btn, background:'#0064ff', borderColor:'#0064ff' }} onClick={handleConfirm}>
              {lang === 'tr' ? '✓ Kaydet ve Koç Panelini Aç' : '✓ Save & Open Coach Dashboard'}
            </button>
          </div>
        )}
      </div>
