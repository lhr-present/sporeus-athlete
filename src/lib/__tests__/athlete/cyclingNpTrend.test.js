...
describe('computeCyclingNpTrend — null cases', () => {
  ...
  it('returns null when log has no bike sessions (only runs/swims)', () => {
    const log = [
      { date: daysAgo(10), type: 'run',  np: 250, duration: 60 },
      { date: daysAgo(20), type: 'swim', np: 180, duration: 30 },
    ]
    expect(computeCyclingNpTrend({ log, today: TODAY })).toBeNull()
  })
  ...
  it('returns null when bike sessions exist but have no NP field', () => {
    const log = [
      { date: daysAgo(10), type: 'bike', duration: 60, tss: 75 },
      { date: daysAgo(20), type: 'ride', duration: 90, tss: 100 },
    ]
    expect(computeCyclingNpTrend({ log, today: TODAY })).toBeNull()
  })
  ...
  it('returns null when bike sessions have NP but no duration', () => {
    const log = [
      { date: daysAgo(10), type: 'bike', np: 250 },
      { date: daysAgo(20), type: 'bike', normalizedPower: 270 },
    ]
    expect(computeCyclingNpTrend({ log, today: TODAY })).toBeNull()
  })
  ...
  it('returns null when all bike sessions are outside the 90d window', () => {
    const log = [
      { date: daysAgo(120), type: 'bike', np: 250, duration: 60 },
      { date: daysAgo(200), type: 'bike', np: 280, duration: 60 },
    ]
    expect(computeCyclingNpTrend({ log, today: TODAY })).toBeNull()
  })
})
...
describe('computeCyclingNpTrend — trend classification', () => {
  it("returns trend='rising' when recent best > early best by ≥3% across buckets", () => {
    const log = [
      ...
      { date: daysAgo(5),  type: 'bike', np: 215, duration: 60 },
    ]
  ...
  it("returns trend='falling' when recent best < early best by ≥3%", () => {
    const log = [
      { date: daysAgo(85), type: 'bike', np: 250, duration: 60 },
      { date: daysAgo(80), type: 'bike', np: 245, duration: 60 },
      { date: daysAgo(55), type: 'bike', np: 240, duration: 60 },
      { date: daysAgo(20), type: 'bike', np: 220, duration: 60 },
  ...
  it("returns trend='stable' when recent vs early are within ±3%", () => {
    const log = [
      { date: daysAgo(85), type: 'bike', np: 250, duration: 60 },
      { date: daysAgo(60), type: 'bike', np: 252, duration: 60 },
      { date: daysAgo(20), type: 'bike', np: 253, duration: 60 },
      { date: daysAgo(5),  type: 'bike', np: 251, duration: 60 },
    ]
    const r = computeCyclingNpTrend({ log, today: TODAY })
    expect(r).not.toBeNull()
...
describe('computeCyclingNpTrend — field handling + bucketing', () => {
  it('accepts both `np` and `normalizedPower` field names', () => {
    const log = [
      { date: daysAgo(80), type: 'bike', np:              200, duration: 60 },
      { date: daysAgo(70), type: 'ride', normalizedPower: 210, duration: 60 },
      { date: daysAgo(10), type: 'bike', normalizedPower: 230, duration: 60 },
    ]
    const r = computeCyclingNpTrend({ log, today: TODAY })
    expect(r).not.toBeNull()
    const b60 = r.buckets.find(b => b.duration === 60)
  ...
  it('buckets are correctly grouped by minimum duration (≥5, ≥20, ≥60)', () => {
    const log = [
      ...
      { date: daysAgo(10), type: 'bike', np: 350, duration: 8 },
      // 25-min effort (counts toward 5+20)
      { date: daysAgo(15), type: 'bike', np: 300, duration: 25 },
  ...
  it('returns a citation matching the published constant', () => {
    const log = [{ date: daysAgo(10), type: 'bike', np: 250, duration: 60 }]
    const r = computeCyclingNpTrend({ log, today: TODAY })
  ...
  it('latestBest reflects the recent-window best at the highest bucket with data', () => {
    const log = [
      ...
      { date: daysAgo(10), type: 'bike', np: 380, duration: 8 },
      // 60-min data
      { date: daysAgo(85), type: 'bike', np: 200, duration: 75 },
      { date: daysAgo(15), type: 'bike', np: 230, duration: 75 },
    ]
    const r = computeCyclingNpTrend({ log, today: TODAY })
  ...
  it('omits buckets that have no qualifying session entirely', () => {
    const log = [
      ...
      { date: daysAgo(10), type: 'bike', np: 320, duration: 7 },
      { date: daysAgo(40), type: 'bike', np: 310, duration: 7 },
    ]
    const r = computeCyclingNpTrend({ log, today: TODAY })
    expect(r).not.toBeNull()
