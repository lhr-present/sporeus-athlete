...
export default function TeamMetrics({ roster }) {
  ...
  const weekLabel = (() => {
    ...
    return `${prev.toLocaleDateString(loc,{month:'short',day:'numeric',timeZone:'UTC'})} – ${d.toLocaleDateString(loc,{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'})}`
  })()

  function handleCopySummary() {
    const lines = lang === 'tr' ? [
      `Sporeus Koç Haftalık — ${weekLabel}`,
      `Takım: ${roster.length} sporcu | bu hafta ${totalThis} antrenman (geçen haftaya göre ${totalThis > totalLast ? '+' : ''}${totalThis - totalLast})`,
      avgReadiness !== null ? `Ort. hazırlık: ${avgReadiness}/100` : null,
      highACWR.length ? `Yüksek ACWR: ${highACWR.map(a => `${a.name} (${a.acwr})`).join(', ')}` : null,
      missedSessions.length ? `Düşük aktivite: ${missedSessions.join(', ')}` : null,
      injured.length ? `Sakatlıklar: ${injured.map(a => `${a.name} (${a.zones.join(', ')})`).join(', ')}` : null,
      '— sporeus.com',
    ].filter(Boolean) : [
  ...
  return (
    <div style={{ ...S.card, marginBottom:'16px', borderLeft:`3px solid #0064ff` }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
        <div style={S.cardTitle}>{lang === 'tr' ? 'HAFTALIK TAKIM ÖZETİ' : 'WEEKLY TEAM SUMMARY'}</div>
        <button style={{ ...S.btnSec, fontSize:'10px', padding:'4px 10px', borderColor:'#0064ff', color:'#0064ff' }} onClick={handleCopySummary}>
          {lang === 'tr' ? 'WhatsApp için Kopyala' : 'Copy for WhatsApp'}
        </button>
      </div>
      <div style={{ ...S.row, marginBottom:'10px' }}>
        {[
          { lbl: lang === 'tr' ? 'BU HAFTA ANTRENMAN' : 'SESSIONS THIS WEEK', val:totalThis, color:'#ff6600', sub:`${totalThis > totalLast ? '↑' : totalThis < totalLast ? '↓' : '='} ${Math.abs(totalThis-totalLast)} ${lang === 'tr' ? 'geçen haftaya göre' : 'vs last wk'}` },
          { lbl: lang === 'tr' ? 'ORT. HAZIRLIK' : 'AVG READINESS', val: avgReadiness !== null ? `${avgReadiness}/100` : '—', color: avgReadiness !== null ? getReadinessColor(avgReadiness) : '#888', sub:'' },
          { lbl: lang === 'tr' ? 'YÜKSEK ACWR' : 'HIGH ACWR', val: highACWR.length, color: highACWR.length ? '#f5c542' : '#5bc25b', sub: highACWR.length ? highACWR.map(a=>a.name).join(', ') : (lang === 'tr' ? 'Sorun yok' : 'All clear') },
          { lbl: lang === 'tr' ? 'SAKATLIKLAR' : 'INJURIES', val: injured.length, color: injured.length ? '#e03030' : '#5bc25b', sub: injured.length ? injured.map(a=>a.name).join(', ') : (lang === 'tr' ? 'Bildirilmedi' : 'None reported') },
